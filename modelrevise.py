...
#         super().__init__()
#         kernel_size = 2 * pad2 + 1
#         self.conv1 = nn.Conv1d(i_channel, o_channel, kernel_size, stride=1,
#                               groups=groups, padding=pad2, bias=False)
#         self.bn2 = nn.BatchNorm1d(o_channel)
#         self.conv2 = nn.Conv1d(o_channel, growth_rate, kernel_size, stride=1,
#                               groups=groups, padding=pad2, bias=False)
...
#         super().__init__()
#         self.layers = layers
#         kernel_size = 2 * pad1 + 1
#         self.conv_in = nn.Conv1d(inc, inc, kernel_size, stride=1,
#                                 padding=pad1, bias=False)
#         self.dense_cnn = nn.ModuleList([
#             EncoderLayer(inc + growth_rate * i, inc + (growth_rate // 2) * i,
#                         growth_rate, groups, pad2)
#             for i in range(layers)
#         ])
#         self.conv_out = nn.Conv1d(inc + growth_rate * layers, outc, kernel_size,
#                                  stride=1, padding=pad1, bias=False)
#         self.relu = nn.ReLU(inplace=True)
#
#     def forward(self, x):
#         x = self.conv_in(x)
#         for layer in self.dense_cnn:
#             x = layer(x)
#         x = self.relu(x)
#         x = self.conv_out(x)
#         return self.relu(x)
...
#     def __init__(self, temperature=0.3, source_number=2):
...
#         self.contrast_loss = SupervisedContrastiveLoss(temperature=0.3, source_number=source_number)
#
#
...
#             loss6 = self.contrast_loss(F.normalize(DTI_normal, p=2, dim=1), contrast_label)
#             # print(loss3)
#             # print(loss5)
#             # print(loss6)
...
#             # ys1 = F.softmax(protein_drug_interaction * 0.4 + dti1d * 0.35 + dti2d * 0.25, 1)  # .to('cpu').data.numpy() C
...
class EncoderLayer(nn.Module):
    def __init__(self, i_channel, o_channel, growth_rate, groups, pad2=7):
        super().__init__()
        self.conv1 = nn.Conv1d(i_channel, o_channel, 2 * pad2 + 1, groups=groups, padding=pad2, bias=False)
        self.bn2 = nn.BatchNorm1d(o_channel)
        self.conv2 = nn.Conv1d(o_channel, growth_rate, 2 * pad2 + 1, groups=groups, padding=pad2, bias=False)
        self.relu = nn.ReLU(inplace=True)
...
class Encoder(nn.Module):
    def __init__(self, inc, outc, growth_rate, layers, groups, pad1=15, pad2=7):
        super().__init__()
        self.layers = layers
        self.conv_in = nn.Conv1d(inc, inc, 2 * pad1 + 1, padding=pad1, bias=False)
        self.dense_cnn = nn.ModuleList([
            EncoderLayer(inc + growth_rate * i, inc + (growth_rate // 2) * i, growth_rate, groups, pad2)
            for i in range(layers)
        ])
        self.conv_out = nn.Conv1d(inc + growth_rate * layers, outc, 2 * pad1 + 1, padding=pad1, bias=False)
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        for layer in self.dense_cnn:
            x = layer(x)
        x = self.relu(x)
        return self.relu(self.conv_out(x))
...
class Fusion(nn.Module):
    ...
    def forward(self, LM_fea, Sty_fea):
        scale_factor = abs(torch.mean(LM_fea)) / abs(torch.mean(Sty_fea))
        Sty_fea_norm = Sty_fea * scale_factor

        f_att = torch.mean(torch.stack([LM_fea.unsqueeze(1), Sty_fea_norm.unsqueeze(1)], dim=1), dim=1)
        f_att = torch.sigmoid(f_att)

        fus_fea = torch.cat([LM_fea, Sty_fea, LM_fea * f_att, Sty_fea * f_att], dim=1)
        return self.combine(fus_fea)
...
class DT_LeNet(nn.Module):
    def __init__(self, hidden, dropout, classes, layers):
        super().__init__()
        self.CNNs = nn.ModuleList([
            nn.Conv1d(hidden, hidden, 7, padding=3) for _ in range(layers)
        ])
        self.FC_combs = nn.ModuleList([
    ...
    def forward(self, dti_feature):
        ...
        for conv in self.CNNs:
            dti_feature = self.act(conv(dti_feature)) + dti_feature
        dti_feature = dti_feature.permute(0, 2, 1).mean(dim=1)

        GRL_feature = dti_feature.clone()
        for fc in self.FC_combs:
            dti_feature = self.act(fc(dti_feature))
        dti_feature = self.FC_down(dti_feature)
        return self.FC_out(dti_feature), dti_feature, GRL_feature
...
class GRL(nn.Module):
    ...
    def backward(self, gradOutput):
        coeff = 2.0 / (1.0 + np.exp(-self.alpha * self.iter_num / self.max_iter)) - 1
        return -coeff * gradOutput
...
class Discriminator(nn.Module):
    ...
    def forward(self, feature1, feature2):
        if self.source_number <= 2:
            return torch.zeros((feature1.shape[0], 1), device=self.device)

        adversarial_out1 = self.grl_layer(feature1.detach())
        adversarial_out2 = self.grl_layer2(feature2)
        adversarial_out1 = torch.matmul(adversarial_out1, nn.init.xavier_uniform_(self.fc1))
        adversarial_out2 = torch.matmul(adversarial_out2, nn.init.xavier_uniform_(self.fc2))

        adversarial_out = adversarial_out2 * torch.sigmoid(adversarial_out1)
        return torch.matmul(torch.relu(adversarial_out), nn.init.xavier_uniform_(self.fc3))
...
class SmoothLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        confidence = 1 - self.smoothing
        logprobs = F.log_softmax(logits, dim=-1)
        nll_loss = -logprobs.gather(-1, labels.unsqueeze(1)).squeeze(1)
        smooth_loss = -logprobs.mean(dim=-1)
        return (confidence * nll_loss + self.smoothing * smooth_loss).mean()


class DGMM_DTI(nn.Module):
    def __init__(self, layer_gnn, device, source_number, hidden1=256, hidden2=75,
                 n_layers=1, attn_heads=1, dropout=0.0):
        super().__init__()
        # GNN components
        self.embed_protein = nn.Embedding(26, hidden2)
        self.W_dnn = nn.ModuleList([nn.Linear(hidden2, hidden2) for _ in range(layer_gnn)])
        self.W_pnn = nn.ModuleList([nn.Linear(hidden2, hidden2) for _ in range(layer_gnn)])
        self.G_A = nn.ModuleList([
            nn.Conv1d(hidden2, hidden2, 3, padding=1, groups=hidden2, bias=False)
            for _ in range(layer_gnn)
        ])
        self.encoder_protein_GNN = Encoder(hidden2, hidden2, 15, 5, groups=1, pad1=15, pad2=7)
        self.bn_layers = nn.ModuleList([
            nn.ModuleList([nn.BatchNorm1d(hidden2) for _ in range(4)])
            for _ in range(layer_gnn)
        ])
        self.gnn_act = nn.GELU()
        ...
        self.dis = Discriminator(100, source_number, device)
        self.lamda = Parameter(torch.tensor([0.8, 0.1, 0.1]))
        self.contrast_loss = SupervisedContrastiveLoss(source_number=source_number)
    ...
    def forward(self, inputs, rand_idx):
        ...
        prot_lm = self.encoder_protein_LM(protein_LM.permute(0, 2, 1)).permute(0, 2, 1)
        mol_lm = self.encoder_drug(molecule_LM.permute(0, 2, 1)).permute(0, 2, 1)
        dt_1d = torch.cat((prot_lm, mol_lm), 1)

        prot_att = self.dropout(self.Informer_blocks[0](dt_1d, prot_lm, prot_lm, None))
        mol_att = self.dropout(self.Informer_blocks[1](dt_1d, mol_lm, mol_lm, None))
        dt_1d_feature = prot_att + mol_att

        # 2D Feature Processing
        prot_gnn = self.embed_protein(proteins.long())
        prot_gnn = self.encoder_protein_GNN(prot_gnn.permute(0, 2, 1)).permute(0, 2, 1)
        dt_2d_feature = self.style_extract(molecule_atoms, molecule_adjs, prot_gnn, self.layer_gnn)
        dt_2d_feature = F.pad(dt_2d_feature, (0, 0, 0, 1300 - dt_2d_feature.size(1)))[:, :1300, :]
        ...
        dti_normal = self.fusion(dti1d_feature, dti2d_feature)
        dti_shuffle = self.fusion(dti1d_feature, dti2d_feature[rand_idx])

        dti = dti_normal.clone()
        for fc in self.DTI_feature:
            dti = self.act(fc(dti))
        dti = self.DTI_Pre(dti)

        lam_dti = self.lamda[0] * dti.detach() + self.lamda[1] * dti1d.detach() + self.lamda[2] * dti2d.detach()
        return (dti, dti1d, dti2d, dti1d_feature, dti2d_feature,
                dti_normal, dti_shuffle, lam_dti, self.lamda, dis_invariant)

    def __call__(self, data, epoch=1, train=True):
        inputs, correct_interaction, sid = data[:-2], data[-2], data[-1]
        correct_interaction = correct_interaction.long().cuda()
        sid = sid.long().cuda()
        rand_idx = torch.randperm(correct_interaction.size(0))

        outputs = self.forward(inputs, rand_idx)
        dti, dti1d, dti2d, dti1d_feature, dti2d_feature, dti_normal, _, lam_dti, _, dis_invariant = outputs

        if not train:
            ys1 = F.softmax(dti * 0.4 + dti1d * 0.27 + dti2d * 0.33, 1)
            ys2 = F.softmax(dti1d, 1)
            ys3 = F.softmax(dti2d, 1)
            return correct_interaction, ys1, ys2, ys3, dti1d_feature, dti2d_feature, dti_normal

        loss_fn = SmoothLoss()
        loss1 = F.cross_entropy(dti, correct_interaction)
        loss2 = F.cross_entropy(dti1d, correct_interaction)
        loss3 = F.cross_entropy(dti2d, correct_interaction)
        loss5 = loss_fn(dis_invariant, sid)
        loss6 = self.contrast_loss(F.normalize(dti_normal, p=2, dim=1), correct_interaction)

        return loss1 + loss2 + loss3 * 1.25 + loss5 + loss6